...
const char* directions[] = {"NE", "NW", "SE", "SW", "N", "S", "E", "W"};
...
void handleWiFiRequests() {
  ...
  if (client) {
    ...
    if (valueIndex != -1) {
      ...
      // Assuming intensity is already scaled to 1-127, we constrain it for safety.
      intensity = constrain(intensity, 1.0, 127.0);
    }
...
void handleSerialInput() {
  if (Serial.available() > 0) {
    ...
    if (motorIndex != -1) {
      ...
      activateMotor(motorIndex, 127.0);  // Use full intensity (max value) for testing
    } else {
...
void activateMotor(int motorIndex, float intensity) {
  ...
  // Use the received intensity directly (already scaled to 1-127)
  uint8_t effectIntensity = (uint8_t)intensity;

  drv.setWaveform(0, effectIntensity); // Set effect intensity based on received value
  drv.setWaveform(1, 0); // End waveform
  drv.go(); // Trigger the effect

  delay(10); // Brief delay for haptic effect
}